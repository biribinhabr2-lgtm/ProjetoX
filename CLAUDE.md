...
- **EventCard precisa de onClick para edição direta**: o card do evento no DayPanel e no EventListView deve ter `onClick={() => onEdit(event)}` no div raiz para que o usuário consiga editar (e escalar equipe) clicando diretamente no card. O dropdown (⋮) precisa de `onClick={(e) => e.stopPropagation()}` no wrapper para não disparar os dois. Sem isso, a única forma de chegar ao formulário de edição é pelo dropdown.
