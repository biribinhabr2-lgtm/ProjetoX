...
function InviteDialog({
  ...
  async function handleGenerate() {
    ...
    setLoading(true)
    try {
      const { createInvite } = await import('@/services/invites')
      const invite = await createInvite(orgId, role, user.id)
      const base   = window.location.origin
      setLink(`${base}/convite/${invite.token}`)
      onSuccess()
    } catch (err) {
  ...
  function handleWhatsApp() {
    if (!link) return
    const msg = `Olá! Você foi convidado para acessar o FestaHub. Clique no link para entrar na equipe:\n${link}`
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-4 py-1">
          {!link ? (
            <>
              <div className="space-y-1.5">
                <Label htmlFor="invite-role">Papel do convidado</Label>
                <select
                  id="invite-role"
                  value={role}
              ...
              <p className="text-xs text-muted-foreground">
                Será gerado um link válido por 7 dias. Compartilhe com quem você quer adicionar à equipe.
              </p>
            </>
          ) : (
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Válido por 7 dias · papel: <strong>{role === 'admin' ? 'Admin' : 'Atendente'}</strong>
              </p>
            </div>
          )}
        </div>
